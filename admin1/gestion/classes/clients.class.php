...
require_once("adresses.class.php");
/**
 *
 */
class Clients
...
#Les déclarations non-ramassées de l'utilisateur connecté
    ...
    public function RenitialiserClient(){
      if (get_class($this)=='ClientLve') {
        $this->connection->query("UPDATE `client_lve` SET `supprime_le`=NULL WHERE `CLIENT_COD`='$this->CLIENT_COD'");
      }else {
...
class ClientLve extends Clients
{
  ...
  public function MesConnexions(){
    return $this->connection->query("SELECT * FROM `connexion` WHERE `id_utilisateur`=$this->CLIENT_ID");
  }
  ...
  #Code de ramassage du carnet en cours et qui n'est pas encore validé
    ...
    public function MesCourriers(){
      require_once "courrier.class.php";
      $c = new Courrier;
      $c->Code1=$this->CLIENT_COD;
      if ($this->CLIENT_TYP == 'TRL') {
        return $c->CourrierLogistique();
      }else {
        return $c->CourrierClient();
      }
    }
  ...
  public function DerniereAdresse(){
    ...
    if ($result){
      foreach ($result as $adres) {
        ...
        if ($adresse->TrouverAdresse($adres['id_adresse'])) {
          return $adresse;
        }else
        return false;
      }

...
<?php if($lang == "ar"){ ?>
$('#tout div.column div.dragbox div.Menu').css('float','right');
<?php } ?>	
  ...
  $('#Menu_<?php echo str_replace("Menu_#","",$row_donnees['contenu']); ?> .sf-menu li').css({'margin' : "0"});
   ...
   <?php } ?>
	...
	if($num_bloc_menu > 0){
				...
				$page_active_menu = "SELECT * FROM menu_active_pages where  id_menu  = '".$row_menu['id']."' AND id_page = '".$id_page."'  LIMIT 1";
					$req_page_active_menu  = mysql_query($page_active_menu ,$connect);
					$num_page_active_menu = mysql_num_rows($req_page_active_menu);
				...
				if(isset($id_page) && ($row_menu['lien'] == $id_page || $num_titre_s_menu > 0 || $num_page_active_menu > 0)){
				...
				$contenu_menu = $contenu_menu.'<a href="javascript:void(0)">';
					...
					if(isset($id_page) && ($row_menu['lien'] == $id_page || $num_titre_s_menu > 0 || $num_page_active_menu > 0)){
					$contenu_menu = $contenu_menu."<img src='".SITEPATH.$row_menu['actif'.$lang_table]."' alt='";
					if($_SESSION['lang']== 'ar'){
					$contenu_menu = $contenu_menu.($row_menu['menu'.$lang_table]);
					}else{
					$contenu_menu = $contenu_menu.($row_menu['menu'.$lang_table]);
						}
					...
					$contenu_menu = $contenu_menu."<img src='".SITEPATH.$row_menu['inactif'.$lang_table]."' alt='".addslashes($row_menu['menu'.$lang_table])."' onmouseover=\"$(this).attr('src','".SITEPATH.$row_menu['survol'.$lang_table]."')\" onmouseout=\"$(this).attr('src','".SITEPATH.$row_menu['inactif'.$lang_table]."')\" /><img src='".SITEPATH.$row_menu['survol'.$lang_table]."' alt='".addslashes($row_menu['menu'.$lang_table])."' style='display:none;' />";
					}
				}else{
					if($row_menu['type'] == 1){
				...
				$contenu_menu = $contenu_menu.'<span>'.($row_menu['menu'.$lang_table]).'</span>';
					}
				}else{
				if($num_s_menu > 0){
				$contenu_menu = $contenu_menu.'<span class="s_menu">'.utf8_decode($row_menu['menu'.$lang_table]).'</span>';
				...
				$page_active_s_menu = "SELECT * FROM menu_active_pages where  id_menu  = '".$row_s_menu['id']."' AND id_page = '".$id_page."'  LIMIT 1";
					$req_page_active_s_menu  = mysql_query($page_active_s_menu ,$connect);
					$num_page_active_s_menu = mysql_num_rows($req_page_active_s_menu);
                

				$contenu_menu = $contenu_menu.'<li';
				if(isset($id_page) && ($row_s_menu['lien'] == $id_page || $num_page_active_s_menu > 0)){
				$contenu_menu = $contenu_menu.' class="current" ';
				}
				$contenu_menu = $contenu_menu.'>';
				...
				$contenu_menu = $contenu_menu.'<a href="javascript:void(0)">';
					...
					$contenu_menu = $contenu_menu."<img src='".SITEPATH.$row_s_menu['actif'.$lang_table]."' alt='";
					if($_SESSION['lang']== 'ar'){
					$contenu_menu = $contenu_menu.($row_s_menu['menu'.$lang_table]);
					}else{
					$contenu_menu = $contenu_menu.($row_s_menu['menu'.$lang_table]);
						}
					...
					$contenu_menu = $contenu_menu."<img src='".SITEPATH.$row_s_menu['inactif'.$lang_table]."' alt='".$row_s_menu['menu'.$lang_table]."' onmouseover=\"$(this).attr('src','".SITEPATH.$row_s_menu['survol'.$lang_table]."')\" onmouseout=\"$(this).attr('src','".SITEPATH.$row_s_menu['inactif'.$lang_table]."')\" /><img src='".SITEPATH.$row_s_menu['survol'.$lang_table]."' alt='".addslashes($row_s_menu['menu'.$lang_table])."' style='display:none;' />";
					}
				}else{
				...
				if($_SESSION['lang']== 'ar'){ 
				$contenu_menu = $contenu_menu.'<span>'.($row_s_menu['menu'.$lang_table]).'</span>';
				}else{
				$contenu_menu = $contenu_menu.'<span>'.($row_s_menu['menu'.$lang_table]).'</span>';
				}
				}

  ...
  return (this.getDay() + 6) %7;
	...
	function Calendar() {
		...
		*/
		...
		this.agendaItemHeight 			= 1;	// height of agend item cell
		...
		this.do_init = function(){
			...
			var firstDayWkIndex = dtFirst.mGetDay();
			// inidex within the week of the last day of the month
			var lastDayWkIndex = dtLast.mGetDay();
		...
		this.renderSingleAgendaItem = function(agi){
			...
			var firstDtIndex = firstRenderDate.mGetDay();
			var lastDtIndex = lastRenderDate.mGetDay();
		...
		this.agendaDropHandler = function(event, ui){
			...
			agendaDiv.fadeOut(function() {
				...
				event.data.endDayDate = newEndDt; // add date that the agenda item was dropped to.
		...
		this.getWeekIndex = function(dayName){
			if(dayName.toUpperCase() == "Dim" || dayName.toUpperCase() == "DIMANCHE"){
				return 0;
			}else if(dayName.toUpperCase() == "Lun" || dayName.toUpperCase() == "LUNDI"){
				return 1;
			}else if(dayName.toUpperCase() == "Mar" || dayName.toUpperCase() == "MARDI"){
				return 2;
			}else if(dayName.toUpperCase() == "Mer" || dayName.toUpperCase() == "MERCREDI"){
				return 3;
			}else if(dayName.toUpperCase() == "Jeu" || dayName.toUpperCase() == "JEUDI"){
				return 4;
			}else if(dayName.toUpperCase() == "Ven" || dayName.toUpperCase() == "VENDREDI"){
				return 5;
			}else if(dayName.toUpperCase() == "Sam" || dayName.toUpperCase() == "SAMEDI"){
				return 6;
			}else{
				return -1
			}			
	...
	// static properties
	Calendar.dayNames = new Array("Lun","Mar","Mer","Jeu","Ven","Sam","Dim");
	/**
	...
	Calendar.showMoreAgendaModal = function(cal,date,agendaItems){
		...
		modal.attr("title","Evénements du " + date.toLocaleString());
		modal.dialog({
			autoOpen: false,
			height: 350,
			width: 400,
		...
		for(var i=0; i<agendaItems.length; i++){
			...
			if(agi.isAllDay()){
				// don't show start time
				agendaDiv.append("(Toute la journée)<br>");
			}else{
				agendaDiv.append( agendaStartDt.toLocaleString() +" à partir de " + DateUtil.getAgendaDisplayTime(agendaStartDt) + "  à " + DateUtil.getAgendaDisplayTime(agendaEndDt) + "<br>");
			}
			// add click event
		...
		this.addAgendaItem = function(calId,title,startDate,endDate,allDay,data,displayProp){
			if(calId != null && title != null && startDate != null && endDate != null && allDay != null){
				...
				if(DateUtil.secondsDifferenceDirection(startDate,endDate) < 0){
					alert("Désolé, vous ne pouvez pas créer un événement qui se termine avant qu'il ne commence");
					return;
				}

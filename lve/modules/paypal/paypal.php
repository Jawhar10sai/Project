...
define('EMAIL_ADD', 'aitelbaz.yassine@gmail.com'); // For system notification.
define('PAYPAL_EMAIL_ADD', 'aitelbaz.yassine-facilitator@gmail.com');
...
switch ($_GET['action']) {
   ...
   default:      // Process and order...
      ...
      // setup a current URL variable for this script
			...
			$url_return =	'http://'.$_SERVER['HTTP_HOST'].'/Panier?validation=ok';
			$url_cancel_return =	'http://'.$_SERVER['HTTP_HOST'].'/Panier?validation=non';
      		$p->add_field('business', PAYPAL_EMAIL_ADD); //don't need add this item. if your set the $p -> paypal_mail.
			$p->add_field('return', $url_return);
			$p->add_field('cancel_return', $url_cancel_return);
			$p->add_field('notify_url', $this_script.'?action=ipn');
			$p->add_field('item_name', 'Paypal Test Transaction');
			$p->add_field('amount', $_POST['amount']);
			$p->add_field('custom', $_POST['custom']);

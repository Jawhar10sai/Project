 ...
 */
	...
	private function log_ipn_results($success) {
		...
		// verification de la commande
			$montant=$this->ipn_data['mc_gross'];
			$commande=$this->ipn_data['custom'];
			
			// query
			$query="Select * from commandes where id=".intval($commande);
			$resultat=mysql_query($query);
			$row=mysql_fetch_row($resultat);
			if(count($row)!=0){
				if($montant==$row[11]){
				$query="update commandes set type='2' where id=".intval($commande);
				mysql_query($query);
				}
			}

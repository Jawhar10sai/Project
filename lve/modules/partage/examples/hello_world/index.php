<?php
	...
	try{
		...
		//$user_profile = $twitter->send_notif();
	//	echo 'FIn Send notification<br>';
		...
		$account_totals = $twitter->getUserContacts();

		// or even inspect it
		//echo "<pre>" . print_r( $user_profile, true ) . "</pre><br />";
		
	//displayTree($account_totals);
		// print recived stats 
		
		foreach($account_totals as $item){
			if($item->email != ''){
			$arrays[] = sprintf('<input type="checkbox" value="%s" name="email">%s', $item->email, $item->email);
			}
		}

		echo "<pre>" . print_r( join('<br/>', $arrays), true ) . "</pre>";

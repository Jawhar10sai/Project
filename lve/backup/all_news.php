...
?>
       ...
       <span style="font-size:10px"><?=dd($row['date'])?></span><br />
        <span class="titre2">- <a href="detail_news.php?id=<?=($row['id'])?>"><?=($row['titre'])?></a></span><br />
        <span style="color:#000000"> <?=substr($row['resume'],0,220)?>...</span><br />
		<br />
        <? }?>
 <?php
...
if ($num > $npp) {
	...
	for ($i = 1; $i <= $j; $i++) {
		...
		if ($n != $i) echo "&nbsp;<a class=link_blue  href=\"all_news.php?r=".$_GET['r']."&type=".$_GET['type']."&cmd=list-news&n={$i}&p={$p}&o={$o}\">{$i}</a>";
	...
	if ($n != $j) echo "&nbsp;<a class=link_blue href=\"all_news.php?r=".$_GET['r']."&type=".$_GET['type']."&cmd=list-news&n=" . ($n + 1) . "&p={$p}&o={$o}\">Suivante</a>";

...
var _manageRowColumns = function(masterId, params) {
  ...
  this.initClassesEvents = function() {
    ...
    this.master.on("click", "." +options.editCell   + " .delete", function(event) { event.stopPropagation(); _this_.deleteCell  (jQuery(this).parents(":eq(2)")) }); // cellSelector/cell/editCell/delete parents("." + options.cellSelector   +":first")); });
    ...
    var notEditableCss  = _id +'.' +options.rowSelector    +' { ' +ps + 'list-style: none; margin: 0;  padding: 0; overflow:hidden;} \r\n'
                        ...
                        + _id +'.' +options.editColumn     +' { ' +pr + 'float:none;' + topLeft + '; height:' + options.dimHiddenSelector  +'px; } \r\n'
    ...
    var editableCss = _id +'.' +options.cell     +' {background-color:transparent;  border :1px ' +options.cellColor +' solid} \r\n'
                    ...
                    + _id +'.' +options.editCell   +' {background-color:' + options.cellColor   +'} \r\n' 
                    + _id +'.' +options.editColumn +' {background-color:' + options.columnColor +'} \r\n' 
                    + _id +'.' +options.editRow    +' {background-color:' + options.rowColor    +'} \r\n' 

                    + _id +'.selected.' +options.cellSelector    +' { background-color:' +options.selectedCellColor   +' } \r\n' 
                    + _id +'.selected.' +options.columnSelector  +' { background-color:' +options.selectedColumnColor +' } \r\n' 
                    + _id +'.selected.' +options.rowSelector     +' { background-color:' +options.selectedRowColor    +' } \r\n'
                    
                    + _id +'.dragOverDrop.' +options.editCell       +' {background-color:' + options.droppableCellColor   +'} \r\n' 
                    + _id +'.dragOverDrop.' +options.columnSelector +' {background-color:' + options.droppableColumnColor +'} \r\n' 
                    + _id +'.dragOverDrop.' +options.rowSelector    +' {background-color:' + options.droppableRowColor    +'} \r\n' 
  ...
  var liveDragDrop = function (target) {
    ...
    var dragOptions = { 
      iframeFix:true,
      opacity: 0.75,
      handle: "." +options.editCell + ",." +options.editColumn + ",." +options.editRow, 
  ...
  var newCell = function(idPage,idBlock,id, title, html, canRemove, canMove, handleState) {  // title must be encoded before
    ...
    var editCellSelector = (handleState != "none") 
        ? "<div class=\"" + options.editCell +"\"><span class=\"title\" title=\"" +title +"\">" +title +"</span>" + divButtons(handleState=="up", false, canRemove) +"</div>" 
        : "";
...
//    return "<li class='" + options.columnSelector  + (canMove ? "" : " noDrag") + "' id='oc_colSel_" + compteur + "' canRemove='" +(canRemove ? "yes" : "no") +"'>" + editColumnSelector(canRemove) + "</li>"; 
  ...
  this.save = function(id_page) {
    ...
    for (var i=0; i<rows.length; i++) {
	 ...
	 var re = new RegExp("#", "g");
	 var texte1 = eltK.find('.contentCell').html();
  vali =   texte1.replace(re, "");
		  
		  
		   $.ajax({
					   url:"insert.php",
					   data:"id_page="+id_page+"&id_block="+eltK.attr('id').substring(eltK.attr('id').lastIndexOf("_")+1)+'&cont_block='+vali,

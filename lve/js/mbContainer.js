...
//						container.get(0).options=this.options;
	...
	jQuery.fn.containerResize = function (){
		...
		container.resizable({
			...
			stop:function(){
				...
				$(container).find('iframe').css("width",$(container).find('.mbcontainercontent').css("width"));
				$(container).find('iframe').css("height",$(container).find('.mbcontainercontent').css("height"));
				container.setContainment();
			}
		});
	...
	jQuery.fn.mb_bringToFront= function(zIndexContext){
		...
		$(this).not(".alwaysOnTop").css('zIndex',999999);
		return zi;
	};

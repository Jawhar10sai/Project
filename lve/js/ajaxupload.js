...
(function($)
{
    ...
    function getURL(settings, fileName, size)
    {
		...
		params.push('ax-file-name=' + (fileName));
    ...
    function fileTemplate(list, fileobj, settings)
    {
	    ...
	    var upload	= $('<a title="Charger" class="ax-upload ax-button" />').click(function(){
	    	if(settings.enable) fileobj.sns();   
	    }).appendTo(toolbar).append('<span class="ax-upload-icon ax-icon"></span>');
	    
	    var remove	= $('<a title="Supprimer" class="ax-remove ax-button" />').click(function(){
    ...
    var globalSettings = 
    {
    	...
    	async:			false,						//set asyncron upload or not
	...
	var methods =
	{
		init : function(options)
		{
    	    ...
    	    {	
				...
				var fieldSet  	= $('<fieldset />').append('<legend class="ax-legend">Ajouter des images</legend>').appendTo($this);
				var browse_c  	= $('<a class="ax-browse-c ax-button" title="Ajouter des images" />').appendTo(fieldSet);//Browse container
				browse_c.append('<span class="ax-plus-icon ax-icon"></span> <span>Ajouter des images</span>');
				
				var browseFiles = $('<input type="file" class="ax-browse" name="ax-files[]" />').attr('multiple',isAjaxUpload).appendTo(browse_c);
				
			    var uploadFiles = $('<a class="ax-upload-all ax-button" title="Charger tout" />').appendTo(fieldSet);
			    uploadFiles.append('<span class="ax-upload-icon ax-icon"></span> <span>Charger tout</span>');
			    
			    var removeFiles = $('<a class="ax-clear ax-button" title="Supprimer tout" />').appendTo(fieldSet);
			    removeFiles.append('<span class="ax-clear-icon ax-icon"></span> <span>Supprimer tout</span>');

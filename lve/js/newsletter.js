			...
			success: function(result) {
				$('#submitting').fadeOut(300, function() {
					if(result == 'yes') {
						$.ajax({
							...
							success: function(emailresult) {
								if (emailresult == 'yes') {
									...
									$('#submitted').fadeIn(300).text('Une erreur s\'est produit lort d\'envoi à votre adresse e-mail.');
									var int = setInterval(function () {
       						 			$('#submitted').fadeOut(300);
										clearInterval(int);
    									}, 5 * 1000);  
						...
						$('#submitted').fadeIn(300).text('Il semble que votre adresse e-mail est déjà dans notre base de données.');
						var int = setInterval(function () {
       						 			$('#submitted').fadeOut(300);
										clearInterval(int);
    									}, 5 * 1000);  
			...
			if($('input[name="emailconf"]').next().attr('id') != "match") {
				...
				$('#match').fadeIn(300, function() {
					...
					if(!isValidEmailAddress(address1)){
						$('#match').after('<div id="bademail">Vos champs correspondent, mais votre adresse e-mail n\'est pas valide. Il doit être formaté comme \'john@oneo.ma\'</div>');

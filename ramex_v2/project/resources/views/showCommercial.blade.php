...
<!-- End of Sidebar -->
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <!--my table begin-->
                <button type="submit" class="btn btn-primary"  href="#" data-toggle="modal" data-target="#addCommercial"><i class="fas fa-plus" style="margin-right:5px"></i>Nouveau commercial</button>
           ...
           <h3 class="text-center">Liste des commerciaux</h3>
           ...
           <thead class="thead thead-dark ">
             ...
             <th>Agence-Commercial</th>
             <th>Secteur-Commercial</th>
             <th>Action</th>
             </tr>
             </thead>
             <tbody id="tabcommercials">
...
<!--my table end-->
    ...
    <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
     ...
     <!--success modal begin-->
      ...
      <!--susccess modal end-->
       ...
       <!--delete motif modal-->
    <div class="modal fade" id="deleteCommercial" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content" style="background-color:#ff0f0f;border-radius:20px;color:white;height:250px"> 
            ...
            <form id="deleteFormID">
             ...
             {{csrf_field()}}
             {{method_field('delete')}}
             <input type="hidden" name="iddelcommercial" id="iddelcommercial" value="">
             <div class="row">
                <div class="col-md-4"> <img src="{{ asset('customAuth/images/warning.gif') }}" alt="" style="width:150px;height:150px"></div>
                <div class="col-md-8" style="padding-top:50px;padding-left:50px"><center><h3>Voulez vous vraiment supprimer ce commercial?</h3></center></div>
            </div>
 ...
 <!--add Commercial modal begin-->
    
<!-- Modal -->
<div class="modal fade" id="addCommercial" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
                     ...
                     <div class="form-group">
                      ...
                      <div class="form-group">
                        <select class="form-control" id="secteur">
                          <option selected disabled>Veuillez choisir un secteur</option>
                        </select>
                        ...
                        <div class="form-group row">
                            <div class="col-sm-12 mb-3 mb-sm-0">
                                <input id="nom" type="text" class="form-control"
                                 name="nom" placeholder="Veuillez entrer un nom" autocomplete="off">
...
<!-- Modal -->
<div class="modal fade" id="editCommercial" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color:#4e73df; color:white">
      ...
      <div class="modal-body">
        ...
        <div class="col-lg-12">
                    ...
                    <form id="editform" enctype="multipart/form-data">
                     ...
                     <div class="form-group">
                        <select class="form-control" id="modagence">
                          <option selected disabled>Veuillez choisir une agence</option>
                        </select>
                      ...
                      <div class="form-group">
                        <select class="form-control" id="modsecteur">
                          <option selected disabled>Veuillez choisir un secteur</option>
                        </select>
                        <small id="modsecteur_error" class="form-text text-danger"></small>
                      </div>

                        <div class="form-group row">
                            <div class="col-sm-12 mb-3 mb-sm-0">
                                <input id="modnom" type="text" class="form-control"
                                 name="modnom" placeholder="Veuillez entrer un nom" autocomplete="off">
                                <small id="modnom_error" class="form-text text-danger"></small>
                            </div>
                        </div>
...
$(document).ready(function(){
    $.get("{{ route('selectAllCommercials') }}", function(data, response){
                
                for (var i = 0; i < data.length; i++) {
                    var line='<tr class="'+data[i].id+'"><td hidden>'+data[i].id+'</td><td>'+data[i].code_commercial+'</td><td>'+data[i].nom_commercial+'</td><td>'+data[i].agence_commercial+'</td><td>'+data[i].secteur_commercial+'</td><td> <a class="btn btn-primary editbtn" href="#" style="margin-right:3px"><i class="fas fa-pen"></i>Modifier</a></button><a class="btn btn-danger delbtn" href="#"  ><i class="fas fa-trash"></i>Supprimer</a></td><tr/>';
                    $('#tabcommercials').append(line);
                }
...
<script>
        $(document).ready( function(){
            ...
            $('#save').on('click',function(e){
              ...
              $.ajax({
                ...
                type:"POST",
                url:"{{route('storeCommercial')}}",
                data:mydata,
                //data:new FormData($('#addform').serialize()),
                success:function(data,response){
                    //console.log(response);
                    $('#addCommercial').modal('hide');
    ...
    <!--Search bar-->
     ...
     <!--modcommercial ajax code-->       
     <script>
           $(document).ready(function(){
            
            $(document).on("click", "a.editbtn" , function() {
            $('#editCommercial').modal('show');
             ...
             $.get("{{ route('AllAgences') }}", function(data, response){
                ...
                for (var i = 0; i < data.length; i++) {
                    if(data[i].nom_agence == $agence){
                        $('#modagence').append('<option value="'+data[i].nom_agence+'" selected>'+data[i].nom_agence+'</option>')
                    } else{
            ...
            //alert(agence);
            $('#modsecteur').html('');
            $.get( url_ , {ag:agence2} , function(data, response){
                var secteurs='<option value="" disabled selected>-- SELECT SECTEUR --</option>'
            for (var i = 0; i < data.length; i++) {
                if(data[i].nom_secteur == $secteur){
                    secteurs = secteurs + '<option value="'+data[i].nom_secteur+'" selected>'+data[i].nom_secteur+'</option>';
                } else{
                    secteurs = secteurs + '<option value="'+data[i].nom_secteur+'">'+data[i].nom_secteur+'</option>';
                }
             }
             $('#modsecteur').append(secteurs);
            })
   ...
   $('#modagence').change( function(){
            ...
            for (var i = 0; i < data.length; i++) {
             ...
             $('#modsecteur').append(secteurs);
              ...
              $('#editform').on('submit',function(e){
                  ...
                  var mydata={
                      id:$('#id').val(),
                      agence: $('#modagence').val(),
                      secteur:$('#modsecteur').val(),
                      nom:$('#modnom').val().toUpperCase(),
                  }
                  //alert(mydata.num_tel);
                 $.ajax({
                     type:"PUT",
                     url:"{{route('updateCommercial')}}",
                     data:mydata,
                     headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    // data:$(this).serialize(),
                     success:function(data,response){
            ...
            <!--delcommercial ajax code-->       
     <script>
           $(document).ready(function(){
            $(document).on("click", "a.delbtn" , function() {
            $('#deleteCommercial').modal('show');
        ...
        /*    $(document).on("click","#delete",function(e){
              ...
              $('#deleteFormID').on('submit',function(e){
               e.preventDefault();
               var id=$('#iddelcommercial').val();

               $.ajax({
                     type:"delete",
                     url:"{{route('deleteCommercial')}}",
                     data:$(this).serialize(),
                     success:function(data,response){
                        $('#deleteCommercial').modal('hide'); 
                        //alert("Utilisateur a été modifié avec succes");
                        //location.reload();

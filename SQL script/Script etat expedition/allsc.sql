...
/****** Object:  StoredProcedure [dbo].[ps_mise_a_jours_etat_expedition]    Script Date: 16/12/2020 16:37:01 ******/
SET ANSI_NULLS ON
GO
...
------------------
------------ table events qui s'aliment avec les triggers dans les table (intervient,COURRIER,  LIVRAISON,COURRIER_ENSEMBLE,TRANSPORTE_ENSEMBLE,TRANSPORTE)
----------------
       ...
       SELECT  @STATUS = CASE @ETAT_FIN
						...
						@STATUS_LIV = CASE @ETAT_FIN
						 ...
						 @LIEU = CASE @ETAT_FIN
												 ...
												 WHEN 4 THEN (
																				CASE WHEN @COLIS_LV = 0 THEN @AGDEPART ELSE @AGARRIVE END
																			  )
											END
...
BEGIN
      SET @STATUS_LIV ='En cours'          
       SELECT TOP 1 @ENSEMBLE_NUM = ENSEMBLE.ENSEMBLE_NUM,
                    @DESTIN_TYP = ISNULL(COURRIER_ENSEMBLE.DESTIN_TYP,(SELECT 
